/**
 * Paginated bookmark fetching with rate limiting, retry, and resume support.
 *
 ...
 *
 * {@link fetchAllBookmarks} additionally persists its cursor + page number
 * into IndexedDB so an interrupted export can be resumed later.
 */

import { graphqlRequest } from './api.js';
import { parseBookmarksPage } from './parser.js';
import { upsertBookmarks, saveExportState, getExportState } from './db.js';
import {
 ...
 *
 * Persists cursor + page number into IndexedDB after every successful page
 * so an interrupted export can be resumed without re-fetching.
 *
 * @param {object} opts
 ...
 * @param {string}   [opts.stateKey='main_bookmarks'] - IndexedDB key prefix for
 *   resume state.
 * @param {number}   [opts.count=20]  - Bookmarks per page.
...
export async function fetchAllBookmarks({
  ...
  let cursor = await getExportState(`${stateKey}_cursor`);
  let pageNum = parseInt(await getExportState(`${stateKey}_page`) || '0', 10);
  let totalTweets = 0;
  let consecutiveEmpty = 0;
  const state = { consecutive429s: 0 };

  if (cursor) {
    onLog?.(`Resuming from page ${pageNum + 1}, cursor: ${cursor.substring(0, 20)}...`, 'info');
  }
  ...
  while (true) {
    ...
    // Persist state for resume after updating cursor.
    await saveExportState(`${stateKey}_cursor`, cursor);
    await saveExportState(`${stateKey}_page`, String(pageNum));

    await sleep(jitteredDelay());
  }

  // Normal completion — clear resume state so next run starts fresh.
  await saveExportState(`${stateKey}_cursor`, null);
  await saveExportState(`${stateKey}_page`, '0');
