 ...
 * @param {string} opts.screenName - Display name / ID for metadata.
 ...
 */
export async function assembleExport({ userId, screenName, startTime }) {
 ...
 * @param {string} screenName - Used in the filename.
 * @returns {string} The generated filename.
 */
export function downloadJSON(data, screenName) {
  const json = JSON.stringify(data, null, 2);
  const blob = new Blob([json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const date = new Date().toISOString().split('T')[0];
  const filename = `xarchive_${screenName || 'bookmarks'}_${date}.json`;
