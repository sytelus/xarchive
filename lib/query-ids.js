...
export async function captureFolderQueryIdViaTab(folderId, userId, onLog) {
  let tabId = null;
  try {
    const folderUrl = `https://x.com/i/bookmarks?folder=${folderId}`;
    onLog?.(`Opening folder ${folderId} in background tab to capture BookmarkFolderTimeline...`, 'info');
    ...
    onLog?.('Timed out waiting for BookmarkFolderTimeline from background tab.', 'warn');
    return null;
  } catch (err) {
    onLog?.(`Folder tab failed: ${err.message || err}`, 'warn');
    return null;
  } finally {
    if (tabId != null) {
      chrome.tabs.remove(tabId).catch(() => {});
