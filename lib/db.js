...
/**
 * Batch-add folder-tweet mappings, skipping entries that already exist.
 *
 * Uses the compound index `[tweet_id+folder_id]` for deduplication
...
export async function addFolderTweets(entries) {
  ...
  // Check each entry against the compound index to filter duplicates.
  const newEntries = [];
  for (const entry of entries) {
    const existing = await db.folder_tweets
      .where({ tweet_id: entry.tweet_id, folder_id: entry.folder_id })
      .first();
    if (!existing) {
  ...
  if (newEntries.length > 0) {
    await db.folder_tweets.bulkAdd(newEntries);
  }
}
